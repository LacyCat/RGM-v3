...
namespace RGM.Donator
{
    public class Main
    {
        ...
        public IEnumerator<float> KillEffect(string kE, Player attacker, Player player, Role _role, Vector3 _pos)
        {
            ...
            if (kE == (en ? "Christmas Tree" : "크리스마스 트리"))
            {
                PlaySound(_pos, "6", 2);
            ...
            if (kE == (en ? "Christmas Ball" : "크리스마스 볼"))
            {
                PlaySound(_pos, "7", 2);
            ...
            if (kE == (en ? "Converging Laser" : "수렴형 레이저"))
            {
                PlaySound(_pos, "9", 2);
