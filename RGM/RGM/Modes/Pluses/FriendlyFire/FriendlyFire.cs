                    ...
                    if (ev.Player.Role is Exiled.API.Features.Roles.Scp173Role scp173)
                    {
                        if (scp173.BlinkCooldown == 0f)
                        {
                            if (ev.Player != player && player.IsScpRole())
                            {
                                ...
                                Server.ExecuteCommand($"/cassie <color=#ff0000>{(en ? player.Role.Name : Trans.Role[ev.Player.Role.Type])}</color>이(가) <color=#ff0000>{(en ? player.Role.Name : Trans.Role[player.Role.Type])}</color>의 뒷통수를 쳤습니다.");
