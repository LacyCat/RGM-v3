...
""";
        ...
        public IEnumerator<float> OnModeStarted()
        {
            ...
            Player.List.ToList().ForEach(x => x.AddBroadcast(20, $"<size=25>🎉 <b><color=yellow>{Player.List.Where(x => x.IsAlive).ToList()[0].DisplayNickname}</color></b>(이)가 <color=#81F79F>눈치게임</color>에서 우승했습니다! 🎉</size>"));
